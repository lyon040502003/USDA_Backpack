...
def search_true_usda_files(usda_files): 
    for file in usda_files:
        ...
        if  os.path.exists(os.path.dirname(os.path.abspath(sublayer_usda_files+file.split("\\")[-2]+"\\" +file.split("\\")[-1]))):
            ...
            os.mkdir(os.path.dirname(os.path.abspath(sublayer_usda_files+file.split("\\")[-2]+"\\" +file.split("\\")[-1])))
            try:
                new_file = open(os.path.abspath(sublayer_usda_files+file.split("\\")[-2]+"\\" +file.split("\\")[-1]),"x")
            except:
                os.remove(os.path.abspath(sublayer_usda_files+file.split("\\")[-2]+"\\" +file.split("\\")[-1]))
                new_file = open(os.path.abspath(sublayer_usda_files+file.split("\\")[-2]+"\\" +file.split("\\")[-1]),"x") 
        #TODO create function to create new usda file
        ...
        for line in opend_file_old:
            ...
            if "file" in line:    
                ...
                #TODO create function to wirte new relative paths in to the file
                ...
                #print(old_line)
                #print(line.replace(old_line, new_rel_file_path))
...
#      /// TEST PINTS ///
print("field found usda ")
print(master_layer_usda_sorce)
print("files found IO ")
